...
def MYAPPNAME():
    from sites.MYAPPNAMEsite import settings
    env.settings = settings
    env.domain = "TODO.example.com"
    env.app_name = "MYAPPNAME"
    env.settings_module = "sites.MYAPPNAMEsite.settings"
    _common()
...
def update_supervisord():
    upload_template("conf_templates/supervisord.conf",
                    ...
                    {
                        "user": env.user,
                        "password": "TODO_SUPERVISOR_PASSWORD", # we're not exposing the port so this isn't so important
